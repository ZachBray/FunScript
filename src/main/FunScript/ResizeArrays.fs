...
let components = 
   [
        ...
        ExpressionReplacer.createUnsafe <@ fun (initValues: seq<_>) -> ResizeArray<_>(initValues)  @> <@ Core.Seq.ToResizeArray @>

        ExpressionReplacer.createUnsafe <@ fun (xs: ResizeArray<_>) -> xs.Count @> <@ Core.ResizeArray.Count @>
        ExpressionReplacer.createUnsafe <@ fun (xs: ResizeArray<_>, index: int) -> xs.Item(index) @> <@ Core.ResizeArray.Item @>
        ExpressionReplacer.createUnsafe <@ fun (xs: ResizeArray<_>, index: int, item: _) -> xs.Item(index) <- item @> <@ Core.ResizeArray.Replace @>
