...
[<AutoOpen>]
module Misc =
    //TODO: This sucks! Can we avoid using exceptions here?
    let mutable isAssertOn = true

    let isVoid (t:Type) =
        let isProviderType() = t.GetType().Name.StartsWith "Provided" 
        let isEqualToVoid() =
           isAssertOn <- false
           let result =
              try t = typeof<System.Void>
              with _ -> false
           isAssertOn <- true
           result
        not (isProviderType()) && isEqualToVoid()
                    ...
                    | :? ProvidedMethod as pminfo   -> 
                      ...
                      let rec emit (callerDontNeedResult : bool) (expr: Quotations.Expr) = 
                          ...
                          | Quotations.Patterns.Call (objOpt,meth,args) -> 
                              ...
                              let returnTypeIsVoid = isVoid meth.ReturnType
                      ...
                      let callerDontNeedResult = isVoid minfo.ReturnType
        ...
        member __.GetInvokerExpression(methodBase, parameters) = 
            ...
            | :? System.Reflection.MethodInfo as minfo ->  
                if minfo.IsStatic then 
                    ...
                    let args = List.tail (Array.toList parameters)
                    Quotations.Expr.Call(parameters.[0], minfo, args)
