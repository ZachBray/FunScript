...
module MapTree = 
   ...
   let current i =
      if i.started then
            match i.stack with
            | MapOne (k,v) :: _ -> new FunJS.Core.LanguagePrimitives.KeyValuePair<_,_>(k,v)
            | []            -> alreadyFinished()
   ...
   type mkIEnumerator'<'Key,'Value when 'Key: comparison>(s) = 
      ...
      interface IEnumerator<FunJS.Core.LanguagePrimitives.KeyValuePair<'Key,'Value>> with 
...
//            | _ -> false
   ...
   interface IEnumerable<FunJS.Core.LanguagePrimitives.KeyValuePair<'Key, 'Value>> with
   ...
   interface System.IComparable with 
      member m.CompareTo(obj: obj) = 
            ...
            Seq.compareWith 
               (fun (kvp1 : FunJS.Core.LanguagePrimitives.KeyValuePair<_,_>) 
                    (kvp2 : FunJS.Core.LanguagePrimitives.KeyValuePair<_,_>)-> 
